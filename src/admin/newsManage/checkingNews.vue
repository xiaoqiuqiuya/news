  ...
  <el-card>
    <div class="top_option">
      <div class="serarch_input">
        <el-input v-model="searchTitle" placeholder="按投稿标题搜索" maxlength="30">
          <el-button slot="append" icon="el-icon-search" @click="btnSearch()"></el-button>
        </el-input>
      </div>
      <el-button class="btn_refresh" type="primary" @click="searchTitle='',getContribute()">刷新</el-button>
      <div class="checkbox_box">
        <el-checkbox
          :indeterminate="selectedStatus"
          v-model="selectAll"
          @change="handelSelectedAllShowItem"
        >全选</el-checkbox>
        <el-checkbox-group v-model="showItem" @change="handelSelectedShowItem">
          <el-checkbox
            v-for="(item,index) in selectShowItem"
            :label="item"
            :key="index"
    ...
    <el-table :data="contributeList" border style="width: 100%">
      ...
      <el-table-column prop="title" label="标题" width="150">
        <template slot-scope="scope">
          <el-popover placement="bottom" trigger="click" :content="scope.row.title">
            <el-link
              type="info"
              slot="reference"
              :underline="false"
      ...
      >
        <template slot-scope="scope">
          <!-- 内容 -->
          <el-button size="mini" v-if="item.prop=='content'" @click="preView(scope.row)">预览</el-button>
          <!-- 文章类型 -->
          <span v-if="item.prop=='original'">
            <el-link v-if="scope.row.original==true" type="primary">原创文章</el-link>
            <el-link v-if="scope.row.original==false" type="warning">转载文章</el-link>
          </span>
          <!-- 原创链接  -->
          <el-link
            v-if="item.prop=='originalLink'"
            type="info"
            slot="reference"
            :underline="false"
            ...
            target="_blank"
            class="span_popover"
          >{{scope.row.originalLink}}</el-link>
          <!-- 描述 -->
          <el-popover
          ...
          <!-- 创建时间 -->
          <span
            v-if="item.prop=='gmtCreate'"
          >{{$moment(scope.row.gmtCreate).format("YYYY-MM-DD HH:ss")}}</span>
          <!-- 修改时间 -->
          <span
            v-if="item.prop=='gmtUpdate'"
          >{{$moment(scope.row.gmtUpdate).format("YYYY-MM-DD HH:ss")}}</span>
        </template>
      </el-table-column>
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-tag type="danger" class="btn_checking" @click="showReBack(scope.row.id)">回退稿件</el-tag>
          <el-tag type="success" class="btn_checking" @click="updateStatus(scope.row.id)">通过审核</el-tag>
        </template>
      </el-table-column>
    </el-table>
    <el-dialog
    ...
    >
      <el-radio-group v-model="selectedObjectionItem">
        <el-radio v-for="item in objectionItem" :key="item.id" :label="item.name">{{item.name}}</el-radio>
      </el-radio-group>
      <el-input v-model="inputObjectionItem"></el-input>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisible = false">取 消</el-button>
        <el-button type="primary" @click="doReBack()">确 定</el-button>
      </span>
    </el-dialog>
    <!-- 文章预览 -->
    <el-dialog title="文章预览" :visible.sync="showContribute">

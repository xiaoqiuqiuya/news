      ...
      <el-col :offset="2"
        ...
        <div class="timeLine_option">
          <el-button type="primary"
                     @click="openAll"
                     class="el-icon-top timeLine_option_btn">全部打开</el-button>
          <el-button type="warning"
                     @click="closeAll"
                     class="el-icon-bottom timeLine_option_btn">全部关闭</el-button>
        </div>
          ...
          <el-collapse v-model="activeNames">
            <el-collapse-item title="添加今日进程"
              ...
              <div>
                <el-form ref="form"
                         :model="timeLineForm">
                  <el-form-item v-for="(web, index) in timeLineForm.webs"
                                :key="web.key">
                    <div class="tag_box"
                         v-if="index == 0">
                      <el-tag>前端</el-tag>
                    </div>
                    <div :class="index == 0 ? 'add_input_item' : 'add_input_item2'">
                      <el-input v-model="web.value"
                                class="my-input"></el-input>
                      <el-button type="primary"
                                 @click.prevent="addInput(1)"
                                 v-if="index == 0">添加项</el-button>
                      <el-button v-else
                                 @click.prevent="removeInput(1, web)"
                                 type="warning">删除</el-button>
                    </div>
                  </el-form-item>
                  <el-form-item v-for="(service, index) in timeLineForm.services"
                                :key="service.key">
                    <div class="tag_box"
                         v-if="index == 0">
                    ...
                    <div :class="index == 0 ? 'add_input_item' : 'add_input_item2'">
                      <el-input v-model="service.value"
                                class="my-input"></el-input>
                      <el-button type="primary"
                                 @click.prevent="addInput(2)"
                                 v-if="index == 0">添加项</el-button>
                      <el-button @click.prevent="removeInput(2, service)"
                                 v-else
                                 type="warning">删除</el-button>
                    </div>
                  </el-form-item>
                  <el-form-item v-for="(mysql, index) in timeLineForm.mysqls"
                                :key="mysql.key">
                    <div class="tag_box"
                         v-if="index == 0">
                    ...
                    <div :class="index == 0 ? 'add_input_item' : 'add_input_item2'">
                      <el-input v-model="mysql.value"
                                class="my-input"></el-input>
                      <el-button type="primary"
                                 @click.prevent="addInput(3)"
                                 v-if="index == 0">添加项</el-button>
                      <el-button @click.prevent="removeInput(3, mysql)"
                                 v-else
                                 type="warning">删除</el-button>
                    </div>
                  </el-form-item>
                  <el-form-item v-for="(issue, index) in timeLineForm.issues"
                                :key="issue.key">
                    <div class="tag_box"
                         v-if="index == 0">
                    ...
                    <div :class="index == 0 ? 'add_input_item' : 'add_input_item2'">
                      <el-input v-model="issue.value"
                                class="my-input"></el-input>
                      <el-button type="primary"
                                 @click.prevent="addInput(4)"
                                 v-if="index == 0">添加项</el-button>
                      <el-button @click.prevent="removeInput(4, issue)"
                                 v-else
                                 type="warning">删除</el-button>
                    </div>
                  </el-form-item>
                  <el-form-item>
                    <div class="add_input_item">
                      <el-button type="primary"
                                 class="my-input-btn"
                                 @click="saveTimeLine">保存进度</el-button>
                    </div>
                  </el-form-item>
                </el-form>
              </div>
            ...
            <el-collapse-item :name="timeLine.date"
                              v-for="timeLine in timeLineList"
                              :key="timeLine.date">
              <div slot="title">
                <el-badge v-if="timeLine.issuesNum != 0"
              ...
              <el-card :body-style="{ padding: '5px' }"
                       ...
                       v-if="timeLine.webs != ''"
                       shadow="hover">
                <p v-for="(web, index) in timeLine.webs"
                   :key="'web' + index">
                  {{ index + 1 }}:{{ web }}
                </p>
              </el-card>
              ...
              <el-card :body-style="{ padding: '5px' }"
                       ...
                       shadow="hover">
                <p v-for="(service, index) in timeLine.services"
                   :key="'service' + index">
                  {{ index + 1 }}:{{ service }}
                </p>
              </el-card>
              <el-card :body-style="{ padding: '5px' }"
                       ...
                       shadow="hover">
                <p v-for="(mysql, index) in timeLine.mysqls"
                   :key="'mysql' + index">
                  {{ index + 1 }}:{{ mysql }}
                </p>
              </el-card>
              <el-card :body-style="{ padding: '5px' }"
                       ...
                       shadow="hover">
                <p v-for="(issue, index) in timeLine.issues"
                   :key="'issue' + index"
                   :class="issue.status ? 'issueOK' : ''">
                  {{ index + 1 }}:{{ issue.content }}
                  <el-button v-if="!issue.status"
                             size="mini"
                             class="btn"
                             @click="issueIsOK(issue.id)">已解决</el-button>
                  <el-button v-if="issue.status"
                             size="mini"
                             class="btn"
  ...
  methods: {
    ...
    openAll() {
      this.timeLineList.forEach((item) => {
        this.activeNames.push(item.date)
      })
    },
    // 全部关闭
    closeAll() {
